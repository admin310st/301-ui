...
## Текущий статус
- **Готово и проверено (Этап 1):**
  - Базовые HTML-формы логина/регистрации на чистом JS (исторический артефакт).
  - Проверка против продового API и Turnstile.
- **Готово (Этап 2):**
  - Перенос на модульный стек Vite + TypeScript; сборка лежит в `public/`.
  - Все auth-флоу синхронизированы с актуальной спецификацией API (login/register/verify/reset/oauth/refresh).
...
2. **Этап 2 — Модульный фронт + Vite** *(выполнен)*
   - Перенос JS в модульную структуру + алиасы для API/форм/утилит.
   - Подключение Vite как dev-server/bundler (без фреймворка) c сборкой в `public/`.
   - Полная TypeScript-типизация auth-эндпойнтов и форм.
   - Обновлённые auth-флоу: Turnstile обязательный, OAuth стартует через GET, `/auth/verify` принимает только `token`.
