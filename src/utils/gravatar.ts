 ...
 */
export async function getGravatarUrl(email: string, size = 120): Promise<string> {
...
async function md5(text: string): Promise<string> {
  // For modern browsers, use SubtleCrypto
  if (crypto.subtle) {
  ...
  // Fallback: simple MD5 implementation for environments without SubtleCrypto
...
function simpleMd5(text: string): string {
  ...
  const rhex = (n: number) => {
    let s = '';
    for (let j = 0; j < 4; j++) s += '0123456789abcdef'.charAt((n >> (j * 8 + 4)) & 0x0f) + '0123456789abcdef'.charAt((n >> (j * 8)) & 0x0f);
    return s;
  };
