/**
 * Redirect filters UI components
 * Manages filter chips, dropdowns, and reset button
 */

import type { FilterConfig, ActiveFilters } from './filters-config';
import { REDIRECT_FILTERS, hasActiveFilters, getDefaultFilters } from './filters-config';
...
export function renderFilterBar(activeFilters: ActiveFilters): string {
  const chips = REDIRECT_FILTERS.map((config) => {
...
export function initFilterUI(
  ...
  container.addEventListener('click', (e) => {
    ...
    const config = REDIRECT_FILTERS.find((f) => f.id === filterId);
    if (!config) return;
