...
- **Sidebar badges/counters:** Добавить счетчики или индикаторы статуса в сайдбар навигацию (видимы в свернутом состоянии).
  - **Вопрос для дизайна:** Какие метрики показывать? Варианты:
    - Количество сущностей (domains: 143, projects: 12, etc.)
    - Статус/уведомления (warnings, errors, pending actions)
    - Непрочитанные события (новые алерты, синхронизации)
  - **Визуальное решение:** Badge позиционируется в правом верхнем углу иконки (уже реализовано в CSS)
  - **Требуется:** Проработать логику данных и UX-паттерн перед реализацией
